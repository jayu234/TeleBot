import requests, schedule, time, threading
import telegram
from fastapi import FastAPI
from telegram.constants import ParseMode

BOT_TOKEN = '8379459708:AAG4b1jBm6B9xQ1c2qIHlMvSEL3QiErpUPY'
CHAT_ID = '5872719081'
DEFAULT_SYMBOL = 'ONGC'

bot = telegram.Bot(token=BOT_TOKEN)
app = FastAPI()

HEADERS = {'User-Agent': 'Mozilla/5.0'}
NSE_API = 'https://www.nseindia.com/api/option-chain-equities?symbol={}'

def fetch_option_chain(symbol):
    session = requests.Session()
    session.get('https://www.nseindia.com', headers=HEADERS)
    res = session.get(NSE_API.format(symbol.upper()), headers=HEADERS)
    res.raise_for_status()
    data = res.json()
    return data['records'], data['filtered']['data']

def analyze(symbol):
    try:
        records, filtered = fetch_option_chain(symbol)
        expiry = records['expiryDates'][0]
        resistance, support = {'strike': None, 'OI': 0}, {'strike': None, 'OI': 0}
        total_call_oi, total_put_oi = 0, 0

        for item in filtered:
            strike = item['strikePrice']
            ce_oi = item.get('CE', {}).get('openInterest', 0)
            pe_oi = item.get('PE', {}).get('openInterest', 0)
            if ce_oi > resistance['OI']: resistance = {'strike': strike, 'OI': ce_oi}
            if pe_oi > support['OI']: support = {'strike': strike, 'OI': pe_oi}
            total_call_oi += ce_oi
            total_put_oi += pe_oi

        pcr = round(total_put_oi / total_call_oi, 2) if total_call_oi else 0
        bias = '📉 Bearish' if pcr < 0.5 else '📈 Bullish' if pcr > 1.2 else '⚖️ Neutral'

        return (
            f\"\\n📈 *{symbol.upper()}* Option Chain\\n🗓️ Expiry: `{expiry}`\\n\"\n
            f\"🔹 Resistance: ₹{resistance['strike']} ({resistance['OI']:,})\\n\"\n
            f\"🔸 Support: ₹{support['strike']} ({support['OI']:,})\\n\"\n
            f\"📊 PCR: `{pcr}`\\n\\n🧭 Bias: {bias}\\n⚠️ Not financial advice.\"
        )
    except Exception as e:
        return f\"❌ Error fetching data for {symbol.upper()}: {str(e)}\"

def send_report():
    message = analyze(DEFAULT_SYMBOL)
    bot.send_message(chat_id=CHAT_ID, text=message, parse_mode=ParseMode.MARKDOWN)

# Scheduler runs in background
def run_schedule():
    schedule.every().day.at(\"09:15\").do(send_report)
    schedule.every().day.at(\"15:40\").do(send_report)
    while True:
        schedule.run_pending()
        time.sleep(60)

# Background thread
threading.Thread(target=run_schedule, daemon=True).start()

# Root route just to keep web service alive
@app.get(\"/\")
def read_root():
    return {\"message\": \"Bot is running\"}
